    ...
    <section id="home" className="relative flex md:min-h-screen pt-28 flex-col items-center justify-center overflow-hidden px-12 text-center">
      ...
      <div className="pointer-events-none absolute inset-0 z-0">
        <img
          ...
          loading="eager"
          decoding="async"
          className="absolute inset-0 h-full w-full object-cover [mask-image:radial-gradient(ellipse_at_center,black_35%,transparent_80%)] opacity-70"
        />
        ...
        {CUBES.map((cube, i) => (
          <div
            key={i}
            className={cn(
              'absolute rounded-xl border border-primary/30 bg-primary/10 backdrop-blur-sm shadow-[0_0_24px_-6px_var(--primary)]',
              cube.className
            )}
            style={{
              animation: `hero-cube-float ${cube.duration} ease-in-out infinite`,
