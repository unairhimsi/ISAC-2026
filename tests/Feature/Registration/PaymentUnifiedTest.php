...
test('bpc can submit payment same gate as olimpiade (legacy SEMIFINAL compat)', function () {
    ...
    $reg = \App\Models\Registration::create(['team_id' => $team->id, 'competition_id' => $c->id, 'batch_id' => $b->id, 'status' => RegistrationStatus::WAITING_PAYMENT, 'team_completed_at' => now(), 'members_completed_at' => now(), 'documents_completed_at' => now(), 'payment_required_at' => now()]);
...
test('submitForVerification requires payment for bpc too', function () {
    ...
    $c = Competition::factory()->create(['type' => Competition::TYPE_BUSINESS_PLAN, 'payment_flow' => Competition::PAYMENT_SEMIFINAL, 'status' => Competition::STATUS_REGISTRATION_OPEN]);
    $b = $c->batches()->create(['name' => 'B1', 'slug' => 'b1-'.uniqid(), 'start_date' => now()->subDay(), 'end_date' => now()->addMonth(), 'price' => 70000, 'quota' => 10, 'status' => BatchStatus::OPEN]);
    \App\Models\Registration::create(['team_id' => $team->id, 'competition_id' => $c->id, 'batch_id' => $b->id, 'status' => RegistrationStatus::WAITING_PAYMENT, 'team_completed_at' => now(), 'members_completed_at' => now(), 'documents_completed_at' => now(), 'payment_required_at' => now()]);
