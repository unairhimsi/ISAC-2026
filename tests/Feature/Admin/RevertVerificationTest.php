...
test('admin can unverify verified team back to waiting_verification', function () {
    ...
    expect(AdminAuditLog::where('action', 'team.unverified')->exists())->toBeTrue();
});
...
test('admin can unverify verified payment back to waiting_verification', function () {
    ...
    $reg = Registration::create(['team_id' => $team->id, 'competition_id' => $c->id, 'batch_id' => Batch::factory()->create(['competition_id' => $c->id])->id, 'status' => RegistrationStatus::VERIFIED, 'payment_required_at' => now(), 'payment_submitted_at' => now(), 'payment_verified_at' => now(), 'paid_at' => now(), 'payment_verified_by' => $admin->id, 'amount_paid' => 70000]);
    $token = $admin->createToken('t')->plainTextToken;
...
test('policy: admin_registration cannot unverify payment', function () {
    ...
    $reg = Registration::create(['team_id' => $regTeam->id, 'competition_id' => $c->id, 'batch_id' => Batch::factory()->create(['competition_id' => $c->id])->id, 'status' => RegistrationStatus::VERIFIED, 'payment_required_at' => now(), 'payment_submitted_at' => now(), 'payment_verified_at' => now(), 'paid_at' => now(), 'payment_verified_by' => $admin2->id, 'amount_paid' => 70000]);
