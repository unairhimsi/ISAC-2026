...
test('advanceStage to semifinal no longer triggers waiting_payment (compat)', function () {
    ...
    $reg = \App\Models\Registration::create(['team_id' => $team->id, 'competition_id' => $c->id, 'batch_id' => $batch->id, 'status' => RegistrationStatus::VERIFIED, 'team_completed_at' => now(), 'members_completed_at' => now(), 'documents_completed_at' => now(), 'submitted_at' => now(), 'payment_submitted_at' => now(), 'payment_required_at' => now(), 'paid_at' => now()]);
...
test('paymentQuery includes legacy SEMIFINAL BPC waiting_payment', function () {
    ...
    $reg = \App\Models\Registration::create(['team_id' => $team->id, 'competition_id' => $c->id, 'batch_id' => Batch::factory()->create(['competition_id' => $c->id])->id, 'status' => RegistrationStatus::WAITING_PAYMENT, 'payment_required_at' => now()]);
    $admin = Admin::factory()->create(['role' => 'super_admin', 'is_active' => true]);
    $token = $admin->createToken('a')->plainTextToken;
