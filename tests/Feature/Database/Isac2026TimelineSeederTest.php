...
test('official timeline seeder is idempotent and stores the exact batch price matrix', function (): void {
    ...
    $expected = [
        'is-olympiad' => ['batch-1' => '60000.00', 'batch-2' => '80000.00'],
        'business-plan-competition' => ['batch-1' => '70000.00', 'batch-2' => '90000.00'],
        'business-it-case-competition' => ['batch-1' => '80000.00', 'batch-2' => '100000.00'],
    ];
...
test('official timeline keeps date-only source dates in Asia Jakarta boundaries and seeds no transaction artifacts', function (): void {
    ...
    $olympiad = Competition::query()->where('slug', 'is-olympiad')->firstOrFail();
    $batchOne = $olympiad->batches()->where('slug', 'batch-1')->firstOrFail();
    $final = $olympiad->stages()->where('name', 'Final')->firstOrFail();

    expect($batchOne->start_date->timezone('Asia/Jakarta')->toDateString())->toBe('2026-08-26')
        ->and($batchOne->end_date->timezone('Asia/Jakarta')->toDateString())->toBe('2026-09-20')
        ->and($final->start_date->timezone('Asia/Jakarta')->toDateString())->toBe('2026-11-22')
